...
def energy_info():
    if request.method == "POST":
        ...
        print("storage")
        return "power_density: " + str(
            power_density) + " " + "wind_speed: " + str(
            wind_speed) + " PVOUT_csi: " + str(
            pvout) + " DNI: " + str(dni) + " GHI: " + str(ghi) + " DIF: " + str(
            dif) + " GTI_opta: " + str(
            gti) + " Class: " + str(class_) + " Head: " + str(
            head) + " Separation: " + str(
            separation) + " Slope: " + str(slope_avg) + " Volume: " + str(
            volume) + " Water to Rock: " + str(
            water_to_rock) + " Energy: " + str(
            energy) + " Storage time: " + str(storage_time) + " Power: " + str(
            power) + " Country: " + (country) + " Distance: " + str(
            grid_distance) + " Elec_price: " + str(elec_price)
...
def get_method_args():
    if request.method == "GET":
        ...
        interest_rate=request.args.get(" interest rate")
        print("csv written")
        print(project_term, itc_on_storage_ststem, sgip_eligible,
              in_state_supplier, sgip_step, saving_assumptions,
              calculation_pattern)
    ...
    if request.method == "POST":
        ...
        itc_on_storage_ststem = request.form.get(
            "itc_on_storage_ststem")  # global itc_on_storage_ststem
        sgip_eligible = request.form.get(
            "sgip_eligible")  # global sgip_eligible
        in_state_supplier = request.form.get(
            "in_state_supplier")  # global in_state_supplier
        saving_assumptions = request.form.get(
            "saving_assumptions")  # global saving_assumptions
...
def calculate_roi_irr():
    ...
    if request.method == "POST":
        ...
        if (calculation_pattern == '1'):
            irr = calculate_IRR_solar_no_storage_model(arr_size_for_irr,
                                                       int(project_term),
                                                       itc_on_storage_ststem_bool,
                                                       sgip_eligible_bool,
                                                       in_state_supplier_bool,
                                                       int(sgip_step),
                                                       saving_assumptions)
            roi = calculate_roi_solar_no_storage(project_term=int(project_term),
                                                 water_area=area_range,
                                                 cost_per_sqm=247.5) / 2

        if (calculation_pattern == '2'):
            irr = calculate_IRR_solar_with_pumped_hydro_model(arr_size_for_irr,
                                                              int(project_term),
                                                              itc_on_storage_ststem_bool,
                                                              sgip_eligible_bool,
                                                              in_state_supplier_bool,
        ...
        if (calculation_pattern == '3'):
            irr = calculate_IRR_solar_with_battery_payback_model(
                arr_size_for_irr, int(project_term),
                itc_on_storage_ststem_bool, sgip_eligible_bool,
                in_state_supplier_bool, int(sgip_step),
                saving_assumptions)
            roi = calculate_roi_solar_with_battery(
                project_term=int(project_term), water_area=area_range,
                cost_per_sqm=247.5)

        if (calculation_pattern == '4'):
            irr = calculate_irr_wind_no_storage(int(project_term),
                                                power_density,
                                                area=arr_size_for_irr,
                                                isOnshore=isOnshore)
            roi = calculate_roi_wind_no_storage(int(project_term),
                                                power_density,
                                                area=arr_size_for_irr,
                                                isOnshore=isOnshore)
            # roi = roi/2
        ...
        if (calculation_pattern == '5'):
            irr = calculate_irr_wind_with_pumped_hydro(int(project_term),
                                                       power_density,
                                                       area=arr_size_for_irr,
                                                       isOnshore=isOnshore)
            roi = calculate_roi_wind_with_pumped_hydro(int(project_term),
                                                       power_density,
                                                       area=arr_size_for_irr,
                                                       isOnshore=isOnshore)

        if (calculation_pattern == '6'):
            irr = calculate_irr_wind_with_battery(int(project_term),
                                                  power_density,
                                                  discount_rate=interest_rate,
                                                  area=arr_size_for_irr,
                                                  isOnshore=isOnshore)
            roi = calculate_roi_wind_with_battery(int(project_term),
                                                  power_density,
        ...
        roi = '{:.4%}'.format(roi)
        irr = '{:.4%}'.format(irr)
        print("####### area_range:" + str(area_range),
              "array_size:" + str(arr_size_for_irr),
...
def calculate_wind_power_density():
    ...
    if request.method == "POST":
        area_range = float(request.json['area'])
        # interest_rate = float(request.json[' interest rate'])
        area0 = area_range
        print(area_range)
...
def calculate_solar_energy(lat, lon, m2, r, PR=1):
    ...
    row = int(row / 432)
    data = np.genfromtxt(
        'Datas/AU Solar ' + str(column) + '-' + str(row) + '.csv',
        delimiter=',')
    out = str("{:.2f}".format(
        float(data[x, y]) * m2 * r * PR / 100)) + " kWh per day at " + str(
        r) + "%' efficiency"
    return out
...
def get_Optimized_Placement_list():
    ...
    if request.method == "POST":
        ...
        optimized_placement_list = generate_rectangle_from_list(
            selected_point_list)
...
def calculate_phes_results():
    try:
        ...
        for key in cm_result.keys():
            if key in ("reservoirs_to_capital", "tunnel_to_capital",
                       "powerhouse_to_capital",
                       "slope", "bank_debt_fraction", "nominal_discount_rate",
                       "real_discount_rate",
                       "npv_refurbishment_to_capital", "cost_to_benchmark",
                       "irr", "roi", "annual_om_rate"):
...
def calculate_head_separation():
    ...
    return jsonify({"code": "1", "msg": "success",
                    "data": {"head": head, "separation": separation}})
